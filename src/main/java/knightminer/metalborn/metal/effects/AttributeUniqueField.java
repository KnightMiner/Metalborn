...
/** Field for the unique key for an attribute. If the key is unset in JSON, defaults to the metal name */
record AttributeUniqueField<P>(String key, Function<P,String> getter) implements LoadableField<String,P> {
  ...
  public String get(JsonObject json, String key, TypedMap context) {
    ...
    if (id == null) {
      throw new JsonParseException("Missing metal ID in context, cannot default " + key);
    }
    return id.getNamespace() + ".metal." + id.getPath();
  }
